...
export function WalletModalContainer() {
  ...
  useEffect(() => {
    if (isConnected && !isWrongNetwork && isWalletModalOpen) {
      ...
      const timer = setTimeout(closeWalletModal, 1500)
  ...
  const handleConnect = useCallback(async (connector: Connector) => {
    try {
      ...
      handleContractError(err, "Wallet connection")
  ...
  const sortedConnectors = useMemo(() => {
    ...
    return uniqueConnectors.sort((a: Connector, b: Connector) => {
      // Prioritize MetaMask and WalletConnect
              ...
              {sortedConnectors.map((connector: Connector) => (
                ...
                >
                  <div className="flex items-center gap-2 sm:gap-3">
                    <img
                      src={WALLET_ICONS[connector.id as keyof typeof WALLET_ICONS] || "/images/wallets/metamask.svg"}
                      alt={connector.name}
                      className="w-5 h-5 sm:w-6 sm:h-6 rounded-full"
                      onError={(e) => {
                        e.currentTarget.src = "/images/wallets/metamask.svg"
                      }}
                    />
